...
function renderCart(items = []) {
    ...
    const html = items
        .map((item) => {
            let additionalHTML = "";
            if(item.gameTypeStr !== "Software") {
                additionalHTML = `<div class="quantity-input mt-2">
            ...
            return `<div class="cart-item d-flex justify-content-between align-items-center">
                ...
                item.gameImg
                    ...
                    <div class="col-md-6 col-lg-6 col-xl-6 px-2">
                        <div>
                            <a href="/game/${item.gameTitle}-${item.gameId}"><h6 class="text-truncate"><span class="badge bg-secondary">${
                item.gameTypeStr
            }</span> ${item.gameTitle}</h6></a>
                        </div>
                        ${additionalHTML}
